 ...
 * 
 */
package corr.resourceManagement.pipelines;
...
import corr.resourceManagement.annotators.WordCounterAnnotator;
import corr.resourceManagement.annotators.WordCounterConsumerAnnotator;
import corr.resourceManagement.annotators.WordSegmenterAnnotator;
import corr.resourceManagement.resources.StopWordModel_Impl;
import corr.resourceManagement.resources.WordCounterModel_Impl;
...
+"Ubuntu est heureuse se qualifier code ouvert. Alors que certains préfèrent voir le code ouvert et le logiciel libre comme deux mouvements en compétition et buts différents, nous nous ne voyons pas ces mouvements comme étant distincts ou incompatibles. Ubuntu comprend fièrement des membres s'identifiant autant au mouvement du logiciel libre que du mouvement du code ouvert, ainsi que de nombreux s'attachant aux deux camps."; 
	...
	public static void main(String[] args) throws Exception {
		...
		ExternalResourceDescription wordCounterResourceDesc = createExternalResourceDescription(WordCounterModel_Impl.class,
				new File("pipo.bin"));

		
		// Binding external resource to each Annotator individually
		AnalysisEngineDescription aed0 = createEngineDescription(WordSegmenterAnnotator.class, WordSegmenterAnnotator.RES_KEY, 
				stopWordsResourceDesc);
		AnalysisEngineDescription aed1 = createEngineDescription(WordCounterAnnotator.class, WordCounterAnnotator.RES_KEY,wordCounterResourceDesc);
		
		AnalysisEngineDescription aed2 = createEngineDescription(WordCounterConsumerAnnotator.class, WordCounterConsumerAnnotator.RES_KEY,
				wordCounterResourceDesc);

		
		// Check the external resource was injected
		AnalysisEngineDescription aaed = createEngineDescription(aed0, aed1, aed2);
